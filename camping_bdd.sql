...
CREATE DEFINER=`root`@`localhost` FUNCTION `checkAjout` (`dh` DATETIME, `duree` INT, `id_ani` INT) RETURNS TINYINT(1)  BEGIN
    ...
    SELECT COUNT(*)+duree into nbheures FROM relation1 inner join creneaux on relation1.id_creneaux = creneaux.id_creneaux WHERE id_animateur = id_ani AND date_heure like date_only;
    IF nbheures > 7 THEN
       SET verif = FALSE;
    END IF;
...
(85, 'Davis', 'David', 'david.newemail@example.com'),
(86, 'Evans', 'Evan', 'test@test.fr'),
(87, 'Evans', 'Evan', 'test@test.fr'),
(89, 'Doe', 'John', 'john.doe@example.com'),
(90, 'Smith', 'Jane', 'jane.smith@example.com'),
(91, 'Davis', 'David', 'david.newemail@example.com'),
(92, 'Evans', 'Evan', 'test@test.fr'),
(93, 'Evans', 'Evan', 'test@test.fr'),
(95, 'Doe', 'John', 'john.doe@example.com'),
(96, 'Smith', 'Jane', 'jane.smith@example.com');
